...
class ReciboGenerator:
    ...
    def _cargar_admin_data(self) -> Dict[str, Dict[str, str]]:
        """Carga datos de administracion en una sola llamada a la API."""
        if self._admin_data_cache is not None:
            return self._admin_data_cache

        try:
            logging.info(
                "[CACHE] Cargando datos de administracion desde Google Sheets..."
            )
            ...
            for registro in registros:
                ...
                if not nombre_inmueble:
                    continue
                admin_data[nombre_inmueble] = {
                    "medio_pago": str(registro.get("medio_pago", "efectivo"))
                    .lower()
                    .strip(),
                    "comision": str(registro.get("comision", "")).strip(),
                    "deposito": str(registro.get("deposito", "")).strip(),
                    "fecha_inicio_contrato": str(
                        registro.get("fecha_inicio_contrato", "")
                    ).strip(),
                }

            self._admin_data_cache = admin_data
            logging.info(
                f"[CACHE] ✓ Cargados {len(admin_data)} registros de administracion"
            )
            return admin_data

        except Exception as e:
            logging.error(f"Error cargando datos de administracion: {e}")
            return {}
    ...
    def _cargar_medios_pago(self) -> Dict[str, str]:
        ...
        admin_data = self._cargar_admin_data()
        medios_pago = {}
        for nombre_inmueble, data in admin_data.items():
            medios_pago[nombre_inmueble] = data.get("medio_pago", "efectivo")

        self._medios_pago_cache = medios_pago
        return medios_pago

    def _obtener_admin_data(self, nombre_inmueble: str) -> Dict[str, str]:
        """Obtiene los datos de administracion desde el cache para una propiedad."""
        try:
            admin_data = self._cargar_admin_data()
            return admin_data.get(nombre_inmueble, {})
        except Exception as e:
            logging.error(
                f"Error obteniendo datos de administracion para {nombre_inmueble}: {e}"
            )
            return {}
    ...
    def _obtener_medio_pago(self, nombre_inmueble: str) -> str:
        ...
        try:
            ...
            if medio_pago == "efectivo" and nombre_inmueble not in medios_pago:
                logging.warning(
                    f"[CACHE] ✗ No se encontró medio_pago para '{nombre_inmueble}', usando 'efectivo'"
                )
            else:
    ...
    def _calcular_mes_actual_contrato(
        self, fecha_inicio_contrato: str
    ) -> Optional[int]:
        """Calcula el mes actual del contrato (1-based) para el periodo del recibo."""
        ...
        for fmt in ("%Y-%m-%d", "%Y_%m_%d"):
            try:
                fecha_inicio = dt.datetime.strptime(fecha_inicio_contrato, fmt).date()
                break
            except ValueError:
                continue
        ...
        try:
            periodo = dt.datetime.strptime(self.mes_periodo, "%Y-%m").date()
        except ValueError:
            return None

        meses_desde_inicio = (periodo.year - fecha_inicio.year) * 12 + (
            periodo.month - fecha_inicio.month
        )
        if meses_desde_inicio < 0:
            return None
        return meses_desde_inicio + 1

    def _estado_cuotas_linea(self, admin_data: Dict[str, str]) -> Optional[str]:
        """Retorna una linea compacta indicando cuotas pagadas si corresponde."""
        if not admin_data:
            return None

        comision = admin_data.get("comision", "")
        deposito = admin_data.get("deposito", "")
        fecha_inicio_contrato = admin_data.get("fecha_inicio_contrato", "")
        mes_actual = self._calcular_mes_actual_contrato(fecha_inicio_contrato)
        if mes_actual != 1:
            return None

        def _is_pagado(config: str) -> bool:
            if not config:
                return False
            if config.lower() == "pagado":
                return True
        ...
        comision_pagada = _is_pagado(comision)
        deposito_pagado = _is_pagado(deposito)

        if not comision_pagada and not deposito_pagado:
            return None

        partes = []
        if comision_pagada:
            partes.append("Comision pagada")
        if deposito_pagado:
            partes.append("Deposito pagado")

        return "Estado cuotas: " + " · ".join(partes)

    def _create_recibo_unificado(
        self, data: Dict, medio_pago: str, incluir_firma: bool = False
    ) -> List:
        ...
        if membrete_path and os.path.exists(membrete_path):
            try:
                ...
                img = Image(
                    membrete_path, width=6 * inch, height=None
                )  # height=None mantiene proporciones
                img.hAlign = "CENTER"
                story.append(img)
        ...
        info_table = Table(
            info_data, colWidths=[1.3 * inch, 2.7 * inch, 0.7 * inch, 1.3 * inch]
        )
        info_table.setStyle(
            TableStyle(
                [
        ...
        desglose_data.append(
            ["Alquiler mensual:", self._format_currency(precio_original)]
        )
        ...
        if descuento and descuento != "0%" and descuento != "0.0%":
            ...
            desglose_data.append(
                [
                    "Descuento:",
                    f"-{self._format_currency(descuento_monto)} ({descuento})",
                ]
            )
        ...
        if cuotas_comision > 0:
            ...
            if "Comisión inmobiliaria" in detalle_cuotas:
                detalle_comision = (
                    detalle_cuotas.split(" + ")[0]
                    if " + " in detalle_cuotas
                    else detalle_cuotas
                )
                desglose_data.append(
                    [detalle_comision + ":", self._format_currency(cuotas_comision)]
                )
        ...
        if cuotas_deposito > 0:
            ...
            if "Depósito en garantía" in detalle_cuotas:
                if " + " in detalle_cuotas:
                    detalle_deposito = (
                        detalle_cuotas.split(" + ")[1]
                        if detalle_cuotas.startswith("Comisión")
                        else detalle_cuotas.split(" + ")[0]
                    )
                else:
                    detalle_deposito = detalle_cuotas
                desglose_data.append(
                    [detalle_deposito + ":", self._format_currency(cuotas_deposito)]
                )

        # Estado de cuotas pagadas (si corresponde)
        admin_data = self._obtener_admin_data(data.get("nombre_inmueble", ""))
        estado_cuotas = self._estado_cuotas_linea(admin_data)
        if estado_cuotas:
            desglose_data.append([estado_cuotas + ":", ""])

        # Servicios adicionales (solo si aplican)
        municipalidad = float(data.get("municipalidad", 0))
        if municipalidad > 0:
            desglose_data.append(
                ["Municipalidad:", self._format_currency(municipalidad)]
            )
        ...
        if medio_pago == "transferencia":
            story.append(
                Paragraph("INSTRUCCIONES DE TRANSFERENCIA", self.styles["Subtitulo"])
            )
            ...
            transferencias_data = [
                [
                    f"Transferir a PROPIETARIO ({data.get('propietario', '')}):",
                    self._format_currency(pago_propietario),
                ],
                [
                    "Transferir a INMOBILIARIA:",
                    self._format_currency(transferencia_inmobiliaria),
                ],
                ["", ""],  # Separador
                ["TOTAL TRANSFERENCIAS:", self._format_currency(precio_final)],
            ]

            transferencias_table = Table(
                transferencias_data, colWidths=[3.5 * inch, 1.5 * inch]
            )
            transferencias_table.setStyle(
                TableStyle(
                    [
                        ("ALIGN", (0, 0), (-1, -1), "LEFT"),
                        ("ALIGN", (1, 0), (1, -1), "RIGHT"),
            ...
            story.append(
                Paragraph(
                    "Entregar el total en efectivo a la inmobiliaria.",
                    self.styles["TextoRecibo"],
                )
            )
        ...
        if actualizacion == "SI":
            ...
            story.append(
                Paragraph(
                    f"✓ Este mes se aplicó actualización: {porc_actual}",
                    self.styles["TextoRecibo"],
                )
            )
        ...
        if meses_prox_ren:
            ...
            if ven_contrato:
                story.append(
                    Paragraph(
                        f"Vencimiento de contrato: {ven_contrato}",
                        self.styles["TextoRecibo"],
                    )
                )
            ...
            if firma_path and os.path.exists(firma_path):
                try:
                    ...
                    firma_content = Table(
                        ...
                        colWidths=[3 * inch],
                        rowHeights=[0.8 * inch, 0.25 * inch, 0.15 * inch],
                    )

                    firma_content.setStyle(
                        TableStyle(
                            [
                                ...
                                (
                                    ...
                                    (0, 0),
                                    "BOTTOM",
                                ),  # Firma alineada abajo de su celda
                                (
                                    "VALIGN",
                                    (0, 1),
                                    (0, 1),
                                    "BOTTOM",
                                ),  # Aclaración alineada abajo de su celda
                                (
                                    "VALIGN",
                                    (0, 2),
                                    (0, 2),
                                    "TOP",
                                ),  # Línea alineada arriba de su celda
                                (
                                    "FONTSIZE",
                                    (0, 1),
                                    (0, 1),
                                    10,
                                ),  # Tamaño para "Victor Abadie"
                                (
                                    "FONTNAME",
                                    (0, 1),
                                    (0, 1),
                ...
                logging.warning(
                    "No se encontró archivo de firma digital en img/firma.png"
                )
                # Fallback a firmas normales
                firmas_data = [
                    ["Inquilino (Locatario)", "Inmobiliaria"],
        ...
        firmas_table = Table(
            ...
            rowHeights=[0.3 * inch, 0.5 * inch, 0.3 * inch],
        )
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (-1, -1),
                        2,
                    ),  # Añadido para controlar espacios
                ]
            )
        )
    ...
    def _create_recibo_efectivo_doble_talon(self, data: Dict) -> List:
        ...
        for destinatario in ["INQUILINO", "INMOBILIARIA"]:
            ...
            if destinatario == "INQUILINO":
                ...
                if membrete_path and os.path.exists(membrete_path):
                    try:
                        ...
                        img = Image(
                            membrete_path, width=5.4 * inch, height=0.9 * inch
                        )  # Reducido 10%
                        img.hAlign = "CENTER"
                        story.append(img)
            ...
            story.append(
                Paragraph(
                    f"RECIBO DE ALQUILER - TALÓN PARA {destinatario}",
                    self.styles["TituloRecibo"],
                )
            )
            ...
            desglose_data.append(["", ""])  # Línea separadora
            desglose_data.append(
                ["TOTAL A PAGAR:", self._format_currency(precio_final)]
            )
            ...
            if actualizacion == "SI":
                ...
                story.append(
                    Paragraph(
                        f"✓ Actualización aplicada: {porc_actual}",
                        self.styles["TextoRecibo"],
                    )
                )
            ...
            if destinatario == "INMOBILIARIA":
                ...
                story.append(
                    Paragraph("INFORMACIÓN PARA INMOBILIARIA", self.styles["Subtitulo"])
                )
                ...
                inmobiliaria_data = [
                    [
                        f"Comisión inmobiliaria: {self._format_currency(comision_total)}",
                        f"Pago al propietario: {self._format_currency(pago_propietario)}",
                    ]
                ]

                inmobiliaria_table = Table(
                    inmobiliaria_data, colWidths=[3 * inch, 3 * inch]
                )
                inmobiliaria_table.setStyle(
                    TableStyle(
                        [
                            ("ALIGN", (0, 0), (-1, -1), "CENTER"),
    ...
    def generar_recibo(self, data: Dict) -> Tuple[str, str]:
        ...
        propietario = self._sanitize_filename(
            data.get("propietario", "Sin_Propietario")
        )
        ...
        try:
            ...
            if medio_pago == "efectivo":
                ...
                doc_sin_firmar = SimpleDocTemplate(
                    str(filepath_sin_firmar), pagesize=A4
                )

            story_sin_firmar = self._create_recibo_unificado(
                data, medio_pago, incluir_firma=False
            )
            ...
            if medio_pago == "efectivo":
                ...
                story_firmado = self._create_recibo_unificado(
                    data, medio_pago, incluir_firma=True
                )
...
def leer_datos_historico(mes_periodo: str) -> List[Dict]:
    ...
    try:
        ...
        logging.info(
            f"Encontrados {len(registros_periodo)} registros para {mes_periodo}"
        )
...
def main():
    ...
    for data in datos_periodo:
        try:
            ...
            logging.error(
                f"Error generando recibo para {data.get('nombre_inmueble', 'Desconocido')}: {e}"
            )
            errores += 1
